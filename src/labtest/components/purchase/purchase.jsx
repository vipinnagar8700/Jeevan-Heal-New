...
const Purchase = (props) => {
  ...
  const getData = () => {
    ...
    fetch("https://jeevan.studiomyraa.com/api/view_labtest", requestOptions)
    .then((res) => res.json())
  ...
  const handleDelete = (id) => {
     fetch('https://jeevan.studiomyraa.com/api/delete_labtest/' + id, { method: 'DELETE' })
        .then((res) => res.json())
  ...
  const columns = [
...

    {
      ...
      cell: row => <Media ><Media.Body className="table-avatar"><Link className="avatar avatar-sm me-2 user-dt" to="#" data-bs-target="#editModal" data-bs-toggle="modal"></Link><span 
			...
			className="user-name">{row.test_name}
    ...
    {
      name: 'Report Time',
      cell: row => row.report_time,//<Media><span className={`btn btn-sm ${row.quantity == "THERE ONLY 7" ? 'bg-danger-light' : row.quantity == "THE ONLY 5" ? 'bg-danger-light' : row.quantity == "THE ONLY 2" ? 'bg-danger-light' : ''}`} >{row.quantity}</span></Media>,
      sortable: true,
      width: "250px",
    },
    // {
...

    {
      ...
      cell: row => (<div className="actions">
        <a
          ...
          href={`/labtest/edit-HealthCheckUpLabtest/${row.id}`}
        >
